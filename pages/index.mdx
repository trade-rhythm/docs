...
Now that you've linked your broker and data provider, you can navigate to the "Research" page in the top navbar to use our various tools to gather insights for all your strategies.
...
To create a strategy using the "No Code" editor, you enter in your "enter" and "exit" conditions using any of our predefined technical indicators as well as (optionally) configure a profit target and stop loss.

### TypeScript / JavaScript

To write your strategy using code, you must implement a two event handlers: `"init"` and `"tick"`.

We do not allow arbitrary javascript modules to be loaded but we do provide a standard library that you can use if you wish. It contains many useful tools (like how to access your broker and data provider) as well as all of our technical indicators. This standard library can be accessed at `https://traderhythm.io/std/v1/mod.js`;
...
addEventListener("init", async () => {
    const fastMA = new EMA(34);
    const slowMA = new EMA(200);
    const crossover = new Cross();
    const crossunder = new Cross();

    const bars = await dataProvider.getBarsBack("1Hour", 200);
